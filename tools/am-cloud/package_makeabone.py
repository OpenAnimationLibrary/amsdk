...
SOURCE_FILES = ('MakeaBone.sln', 'MakeaBone.vcxproj', 'MakeaBone.cpp',
                'MakeaBone.rc', 'resource.h', 'StdAfx.h', 'Coordinates.h',
                'CoordinatesTests.cpp', 'README.md')
...
def package(native_zip: Path, output: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory() as temporary:
        root = Path(temporary) / 'MakeaBone-0.1.0'
        native = root / 'plugin'
        ...
        if (receipt['status'] != 'passed' or receipt['configuration'] != 'Release' or
                receipt['plugin']['file'] != 'MakeaBone_64.hxt' or
                receipt['source']['checkout_commit'] != commit):
            raise ValueError('Expected this checkout\'s audited MakeaBone Release package.')
        verify_hash(native / 'MakeaBone_64.hxt', receipt['plugin']['sha256'])
        source = root / 'source'
        source.mkdir()
        for name in SOURCE_FILES:
            shutil.copy2(HERE / 'plugins/MakeaBone' / name, source / name)
        ...
        kit = output / f'MakeaBone-0.1.0-Release-x64-r{run}-{commit[:12]}-kit.zip'
