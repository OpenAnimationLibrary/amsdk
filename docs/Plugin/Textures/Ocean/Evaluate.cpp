// RC  10/1/2001  \Randy90\Plugin\Textures\Ocean\Evaluate.cpp
...
float CustomTexture::fBm(const Vector P, const float octaves, const float gain) {
    float amp = 1.F;
    Vector pp = P / gain;
    float sum = 0.F;
...
float CustomTexture::fracSum(const Vector P, const float octaves) const {
    const float lac = m_wlac->GetValue() / 10.F;
    const float gain = m_wgain->GetValue() / 10.F;
    float Km = m_amplitude->GetValue();
    const float wamp = m_wamp->GetValue() / 100.F;
    //    float angle = gain; //*TWOPI;
    float sum = 0.F;
    //float ncx=lac*TWOPI;

    Vector wav(lac, 0.F, gain);
    //P.Normalize();
    for (int i = 0; i < octaves; ++i) {
        Vector ii((float)i);
        sum += Km * hash_math::cos(P.Dot(wav));
        wav.x += snoise(ii) * wamp;
        wav.z += snoise(-ii) * wamp;
...
bool CustomTexture::Evaluate(const Vector &evalp, HTexInfo *texinfo) {
    ...
    const float noise = m_wo->GetValue();
    Vector e;
    ...
    const float bumpness = m_noise->GetValue() / 10.F;
    float mix = m_mix->GetValue() / 100.F;
    const float octaves = m_freq->GetValue();
    float noiz = 0.F;
    ...
    ca.y = e.y;
    ca.z = e.z;
    float x = -fracSum(ca, noise);
    if (bumpness > 0.F) {
        const Vector ee = evalp;
        noiz = bumpness * fBm(ee + m_offset->GetZ()->GetValue() / 10.F, octaves, mix);
        x += noiz;
    }
    //
