...
CPostApp::CPostApp(): m_styleinfo{nullptr}, m_percentinfo1{nullptr}, m_percentinfo2{nullptr} {}
...
BOOL CPostApp::InitInstance() {
    ...
    m_styletypes[SCREEN]->SetMatchName("Screen");
    string.LoadString(IDS_SCREEN);
    m_styletypes[SCREEN]->SetLocalizedName(string.GetBuffer());

    m_styletypes[OVERLAY]->SetMatchName("Overlay");
    string.LoadString(IDS_OVERLAY);
    m_styletypes[OVERLAY]->SetLocalizedName(string.GetBuffer());

    string.LoadString(IDS_STYLE);
    m_styleinfo = HTypePropertyInfo::New(string, "Style", &m_styletypes);
    m_styleinfo->SetDefaultIndex(SCREEN);
