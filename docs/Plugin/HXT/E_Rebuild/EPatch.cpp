...
void eGetPatchMid_4(EPatch* ep) {
    ...
    mp_temp *= 0.25F;
    pos[8] = (pos[1] + pos[3] + pos[5] + pos[7]) * 0.25F;
...
void eGetPatchMid_5(EPatch* ep) {
    ...
    mp_temp *= 0.2F;
    pos[10] = (pos[1] + pos[3] + pos[5] + pos[7] + pos[9]) * 0.2F;
...
void eGetNormal_4(EPatch* ep) {
    ...
    if (ep->opatch->GetCP4()->IsFlip())
        normal4.Negate();
    ep->ndir[4] = normal4;
    const Vector normal = (normal1 + normal2 + normal3 + normal4) * 0.25F;
    ep->ndir[0] = normal;
}
...
void eGetNormal_5(EPatch* ep) {
    ...
    if (ep->opatch5->GetCP5()->IsFlip())
        normal5.Negate();
    ep->ndir[5] = normal5;
    const Vector normal = (normal1 + normal2 + normal3 + normal4 + normal5) * 0.2F;
    ep->ndir[0] = normal;
}
