...
void FSWFStream::WriteBits(const U32 data, U32 size) //adds individual bits
{
...
void FSWFStream::WriteLargeData(const U8 *data, const U32 size) {
    LargeData large{};
...
void FSWFStream::WriteDWord(const U32 data) {
...
void FSWFStream::WriteWord(const U32 data) {
...
void FSWFStream::WriteByte(const U32 data) {

    //output the byte
    const U32 v = data << 24 >> 24;
    WriteBits(v, 8);
...
U32 FSWFStream::Size() {
    ...
    for (std::list<LargeData>::iterator it = outDataList.begin(); it != outDataList.end(); ++it) {
        const LargeData &data = *it;
        size += data.size;
    }
    return size;
}
...
void FSWFStream::WriteToFile(FILE *swfFile) {
    ...
    const U32 outDataListSize = outDataList.size();

    [[maybe_unused]] int wrote = 0;

    if (outDataListSize) {

        for (U32 i = 0; i < outDataListSize; i++) {
            const U32 currentDataPosition = outDataList.front().position;
            const U8 *currentData = outDataList.front().data;
            const U32 currentDataSize = outDataList.front().size;
            outDataList.pop_front();

            FLASHASSERT(currentDataPosition >= currentStreamPos);

            if (currentDataPosition - currentStreamPos > 0) {
                [[maybe_unused]] auto numw0 = fwrite(&stream[currentStreamPos], 1, currentDataPosition - currentStreamPos, swfFile);
            }
            ...
            [[maybe_unused]] auto numw1 = fwrite(currentData, 1, currentDataSize, swfFile);
            wrote += currentDataSize;
        }
    }

    if (streamPos > currentStreamPos) {
        [[maybe_unused]] auto numw2 = fwrite(&stream[currentStreamPos], 1, streamPos - currentStreamPos, swfFile);
    }
...
void FSWFStream::AppendTag(const U16 tagID, U32 length, FSWFStream *buffer) {
    U32 longLength = 0;
    bool longHead = false;
    ...
    const U16 firstPartOfTag = tagID << 6 | length; // Build up the first 2 bytes of the tag:
    //		10bits for tag ID 
...
U32 FSWFStream::MinBits(const U32 number, const U16 sign) {
...
U32 FSWFStream::MaxNum(const S32 a, const S32 b, const S32 c, const S32 d) {

    //take the absolute values of the given numbers
    const int aAbs = abs(a);
    const int bAbs = abs(b);
    const int cAbs = abs(c);
