...
void CreateOvalMovie() {
    ...
    auto *rectBounds = new FRect(0, 0, 4000, 2000);
    auto *oval = new FDTDefineShape(rectBounds);

    //record its ID so that we can later refer to it
    const U16 ovalID = oval->ID();

    //construct a blue FColor
    //construct a solid fill style of the given color
    const U32 blueFillID = oval->AddSolidFillStyle(new FColor(0, 0, 0xff));
    ...
    const U32 blackLineStyleID = oval->AddLineStyle(20, new FColor(black));
    ...
    ovalShapeRecords[0] = new FShapeRecChange(false, true, true, false, true, 3414, 1707, 0, blueFillID, blackLineStyleID, nullptr, nullptr);
    ...
    for (auto &ovalShapeRecord: ovalShapeRecords)
        oval->AddShapeRec(ovalShapeRecord);
    ...
    auto *placeoval = new FCTPlaceObject2(false, // ~ _hasClipDepth
                                          false,
                                          true,
                                          false,
                                          1,
