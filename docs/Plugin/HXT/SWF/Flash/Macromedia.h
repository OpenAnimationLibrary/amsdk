...
// 
// 	#define FLASHOUTPUT (()0)
// 	#define FLASHASSERT (()0)
// 	#define FLASHPRINT  (()0)
#else
    inline void nothing(...) {}
    #define FLASHOUTPUT nothing       //(()0) 
	#define FLASHASSERT nothing       //(()0) 
	#define FLASHPRINT  printf
#endif
...
constexpr SFIXED Fixed1 = 0x00010000;
constexpr SCOORD SCoord1 = 20;
...
constexpr U8 Snd5k = 0;
constexpr U8 Snd11k = 1;
constexpr U8 Snd22k = 2;
constexpr U8 Snd44k = 3;

constexpr U8 Snd8Bit = 0;
constexpr U8 Snd16Bit = 1;

constexpr U8 SndMono = 0;
constexpr U8 SndStereo = 1;
...
enum {
    ...
    // A bitmap using lossless zlib compression.
    stagDefineBitsJPEG2 = 21,
    // A bitmap using an internal JPEG compression table.
    stagDefineShape2 = 22,
    stagDefineButtonCxform = 23,
    stagProtect = 24,
    // This file should not be importable for editing.

    stagPathsArePostScript = 25,
    // assume shapes are filled as PostScript style paths

    // These are the new tags for Flash 3.
    stagPlaceObject2 = 26,
    // The new style place w/ alpha color transform and name.
    ...
    stagSyncFrame = 29,
    // Handle a synchronization of the display list
    ...
    // A button V2 includes color transform, alpha and multiple actions
    stagDefineBitsJPEG3 = 35,
    // A JPEG bitmap with alpha info.
    stagDefineBitsLossless2 = 36,
    // A lossless bitmap with alpha info.
    stagDefineSprite = 39,
    // Define a sequence of tags that describe the behavior of a sprite.
    stagNameCharacter = 40,
    // Name a character definition, character id and a string, (used for buttons, bitmaps, sprites and sounds).

    stagSerialNumber = 41,
    // a tag command for the Flash Generator customer serial id and cpu information
    stagDefineTextFormat = 42,
    // define the contents of a text block with formating information
    ...
    stagDefineFont2 = 48,
    // a tag command for the Flash Generator Font information
    stagGenCommand = 49,
    // a tag command for the Flash Generator intrinsic
    stagDefineCommandObj = 50,
    ...
    // an edit text object (bounds, width, font, variable name)
    stagDefineVideo = 38,
...
enum {
    splaceMove = 0x01,
    // this place moves an exisiting object
    splaceCharacter = 0x02,
    // there is a character tag	(if no tag, must be a move)
    splaceMatrix = 0x04,
    // there is a matrix (matrix)
    splaceColorTransform= 0x08,
    // there is a color transform (cxform with alpha)
    splaceRatio = 0x10,
    // there is a blend ratio (word)
    splaceName = 0x20,
    // there is an object name (string)
    splaceDefineClip = 0x40,
    // this shape should open or close a clipping bracket (character != 0 to open, character == 0 to close)
...
enum {
    ...
    // frame needed (WORD), actions to skip (BYTE)
...
enum {
    sbitsAlphaFlag = 0,
    // just a flag that the alpha channel is valid
    sbitsAlphaCTab = 1,
    // alpha values for a color table
...
enum {
    ...
    spktMask = 0x03,
    spktResend = 0x04,
    // flags for object packets

    spktSeekPoint = 0x04,
    // flags for frame packets
    spktKeyFrame = 0x08
...
enum {
    ...
    // end of text flag
    stextStyle = 0x80,
    // font style: followed by 8-bit flags (bold, italic, etc...)
    stextFont = 0x81,
    // font identifier: followed by 16-bit font identifier
    stextSize = 0x82,
    // font size: followed by 16-bit value in twips
    stextColor = 0x83,
    // font color: followed by 32-bit RGB value
    stextPosition = 0x84,
    // font position: followed by 8-bit position (normal, super or subscript)
    stextKerning = 0x85,
    // font kerning: followed by 16-bit kerning value in twips
    stextReserved1 = 0x86,
    // reserved value
    stextReserved2 = 0x87,
    // reserved value
    stextAlignment = 0x88,
    // paragraph alignment: followed by 8-bit alignment value
    stextIndent = 0x89,
    // paragraph alignment: followed by 16-bit indent value in twips
    stextLMargin = 0x8a,
    // paragraph left margin: followed by 16-bit left margin value in twips
    stextRMargin = 0x8b,
    // paragraph right margin: followed by 16-bit right margin value in twips
    stextLeading = 0x8c,
    // paragraph leading: followed by 16-bit leading value in twips
    stextReserved3 = 0x8d,
    // reserved value
    stextReserved4 = 0x8e,
    // reserved value
