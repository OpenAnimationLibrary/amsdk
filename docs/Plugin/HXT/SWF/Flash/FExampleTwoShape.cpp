...
void CreateTwoShapes() {
    ...
    auto *rectBounds1 = new FRect(0, 0, 2000, 2000); //coordinate values are in TWIPS

    //construct the FDTDefineShape which will be the rectangle image
    auto *rectangle1 = new FDTDefineShape(rectBounds1);

    //record its ID so that we can later refer to it
    const U16 rectangleID1 = rectangle1->ID();
    ...
    const U32 redfillID = rectangle1->AddSolidFillStyle(new FColor(red));

    //construct a black color
    const FColor black1 = FColor(0, 0, 0);
    ...
    const U32 blackLineStyleID1 = rectangle1->AddLineStyle(20, new FColor(black1));
    ...
    rectangleShapeRecords1[0] = new FShapeRecChange(false, true, true, false, true, 2000, 0, 0, redfillID, blackLineStyleID1, nullptr, nullptr);
    ...
    for (auto &i: rectangleShapeRecords1)
        rectangle1->AddShapeRec(i);
    ...
    auto *placeRectangle1 = new FCTPlaceObject2(false, // ~ _hasClipDepth
                                                false,
                                                true,
                                                false,
                                                1,
    ...
    //construct a rect that defines the shape's bounds 
    auto *rectBounds2 = new FRect(1000, 1000, 3000, 3000);

    //construct the FDTDefineShape which will be the rectangle image
    auto *rectangle2 = new FDTDefineShape(rectBounds2);

    //record its ID so that we can later refer to it
    const U16 rectangleID2 = rectangle2->ID();
    ...
    //so that you can later refer to it.  The AddFillStyle function of fillStyle 
    //array returns the position so record that in a field called fillID
    const U32 bluefillID = rectangle2->AddSolidFillStyle(new FColor(blue));

    //construct a black color
    const FColor black2 = FColor(0, 0, 0);

    //add a black, 1 pixel (20 TWIPS) wide line style to rectangle2, remembering to store the
    // position of the line style just as in the fill style.
    const U32 blackLineStyleID2 = rectangle2->AddLineStyle(20, new FColor(black2));
    ...
    rectangleShapeRecords2[0] = new FShapeRecChange(false, true, true, false, true, 3000, 1000, 0, bluefillID, blackLineStyleID2, nullptr, nullptr);
    //Create straight edge object (just a stuct of info), store it in EdgeRecord
    ...
    //Add the shape records to the rectangle shape object
    for (auto &k: rectangleShapeRecords2)
        rectangle2->AddShapeRec(k);

    //Add the rectangle to the given object collection
    allTags.AddFObj(rectangle2);

    //create a place object tag which puts the rectangle on the display list
    auto *placeRectangle2 = new FCTPlaceObject2(false, // ~ _hasClipDepth
                                                false,
                                                true,
                                                false,
                                                1,
