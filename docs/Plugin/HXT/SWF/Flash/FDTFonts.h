...
public:
    FDTDefineFont();
    ~FDTDefineFont() override;

    U16 ID() override;
    void AddShapeGlyph(FShape *_shape);

    [[nodiscard]] int NumberOfGlyphs() const {
        return shapeGlyphs.size();
    }
...
public:
    ...
    FDTDefineFont2(const char *_fontName, U16 _encodeType, U16 _italicFlag, U16 _boldFlag, S16 _ascenderHeight, S16 _descenderHeight, S16 _leadingHeight);
    ~FDTDefineFont2() override;

    void AddShapeGlyph(FShape *_shape, U16 _shapeCode, S16 _shapeAdvance = 0, FRect *_shapeBounds = nullptr);
    void AddKerningRec(FKerningRec *_kerningRecord);
    [[nodiscard]] U16 nIndexBits() const;
    U16 ID() override;
    void WriteToSWFStream(FSWFStream *_SWFStream) override;
...
public:
    FDTDefineFontInfo(const char *_fontName, U16 _fontID, U16 _encodeType, U16 _italicFlag, U16 _boldFlag);
    ~FDTDefineFontInfo() override;
