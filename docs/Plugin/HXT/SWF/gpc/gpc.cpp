...
typedef struct v_shape /* Internal vertex list datatype     */ {
    ...
    v_shape *next; /* Pointer to next vertex in list    */
} vertex_node;
...
typedef struct p_shape /* Internal contour / tristrip type  */ {
    ...
    p_shape *next; /* Pointer to next polygon contour   */
    p_shape *proxy; /* Pointer to actual structure used  */
} polygon_node;
...
typedef struct edge_shape {
    ...
    edge_shape *prev; /* Previous edge in the AET          */
    edge_shape *next; /* Next edge in the AET              */
    edge_shape *pred; /* Edge connected at the lower end   */
    edge_shape *succ; /* Edge connected at the upper end   */
    edge_shape *next_bound; /* Pointer to next bound in LMT      */
} edge_node;
...
typedef struct lmt_shape /* Local minima table                */ {
    ...
    lmt_shape *next; /* Pointer to next local minimum     */
} lmt_node;

typedef struct sbt_t_shape /* Scanbeam tree                     */ {
    double y; /* Scanbeam node y value             */
    sbt_t_shape *less; /* Pointer to nodes with lower y     */
    sbt_t_shape *more; /* Pointer to nodes with higher y    */
} sb_tree;
...
typedef struct it_shape /* Intersection table                */ {
    ...
    it_shape *next; /* The next intersection table node  */
} it_node;
...
typedef struct st_shape /* Sorted edge table                 */ {
    ...
    double xt; /* Scanbeam top x coordinate         */
    double dx; /* Change in x for a unit y increase */
    st_shape *prev; /* Previous edge in sorted list      */
} st_node;
...
static edge_node **bound_list(lmt_node **lmt, const double y) {
...
static void add_to_sbtree(int *entries, sb_tree **sbtree, const double y) {
    if (!*sbtree) {
...
static int count_optimal_vertices(const gpc_vertex_list c) {
    int result = 0;
...
static edge_node *build_lmt(lmt_node **lmt, sb_tree **sbtree, int *sbt_entries, gpc_polygon *p, const int type, const gpc_op op) {
    int c, i, min, max, num_edges, v;
...
static void add_intersection(it_node **it, edge_node *edge0, edge_node *edge1, const double x, const double y) {
...
static void add_st_edge(st_node **st, it_node **it, edge_node *edge, const double dy) {
    ...
    if (!*st) {
        ...
        const double den = (*st)->xt - (*st)->xb - (edge->xt - edge->xb);
        ...
        if (edge->xt >= (*st)->xt || edge->dx == (*st)->dx || fabs(den) <= DBL_EPSILON) {
            ...
            const double r = (edge->xb - (*st)->xb) / den;
            const double x = (*st)->xb + r * ((*st)->xt - (*st)->xb);
            const double y = r * dy;
...
static void build_intersection_table(it_node **it, edge_node *aet, const double dy) {
    st_node *st;
...
static void add_left(polygon_node *p, const double x, const double y) {
    vertex_node *nv;
...
static void merge_left(polygon_node *p, polygon_node *q, polygon_node *list) {
    ...
    if (p->proxy != q->proxy) {
        ...
        for (const polygon_node *target = p->proxy; list; list = list->next) {
...
static void add_right(polygon_node *p, const double x, const double y) {
    vertex_node *nv;
...
static void add_local_min(polygon_node **p, edge_node *edge, const double x, const double y) {
    vertex_node *nv;
...
static void add_vertex(vertex_node **t, const double x, const double y) {
    if (!*t) {
...
static void new_tristrip(polygon_node **tn, edge_node *edge, const double x, const double y) {
    if (!*tn) {
...
static void minimax_test(gpc_polygon *subj, gpc_polygon *clip, const gpc_op op) {
    int s, c, *o_table, overlap;

    bbox *s_bbox = create_contour_bboxes(subj);
    bbox *c_bbox = create_contour_bboxes(clip);
...
void gpc_free_polygon(gpc_polygon *p) {
    ...
    for (int c = 0; c < p->num_contours; c++) FREE(p->contour[c].vertex)
    FREE(p->hole)
    FREE(p->contour)
    p->num_contours = 0;
}

void gpc_read_polygon(FILE *fp, const int read_hole_flags, gpc_polygon *p) {

    [[maybe_unused]] int numread = fscanf_s(fp, "%d", &p->num_contours);
    ...
    for (int c = 0; c < p->num_contours; c++) {
        numread = fscanf_s(fp, "%d", &p->contour[c].num_vertices);

        if (read_hole_flags)
            numread = fscanf_s(fp, "%d", &p->hole[c]);
        else
        ...
        for (int v = 0; v < p->contour[c].num_vertices; v++) {
            numread = fscanf_s(fp, "%lf %lf", &p->contour[c].vertex[v].x, &p->contour[c].vertex[v].y);
        }
    }
}

void gpc_define_4corner(gpc_polygon *p, const int x1, const int y1, const int x2, const int y2, const int x3, const int y3, const int x4, const int y4) {
    p->num_contours = 1;
    MALLOC(p->hole, 1 * sizeof(int), "hole flag array creation", int)
    MALLOC(p->contour, 1 * sizeof(gpc_vertex_list), "contour creation", gpc_vertex_list)
    ...
    p->contour[0].vertex[0].x = x1;
    p->contour[0].vertex[0].y = y1;
    p->contour[0].vertex[1].x = x2;
    p->contour[0].vertex[1].y = y2;
    p->contour[0].vertex[2].x = x3;
    p->contour[0].vertex[2].y = y3;
    p->contour[0].vertex[3].x = x4;
    p->contour[0].vertex[3].y = y4;
}

void gpc_define_3corner(gpc_polygon *p, const int x1, const int y1, const int x2, const int y2, const int x3, const int y3) {
    p->num_contours = 1;
    MALLOC(p->hole, 1 * sizeof(int), "hole flag array creation", int)
    MALLOC(p->contour, 1 * sizeof(gpc_vertex_list), "contour creation", gpc_vertex_list)

    p->hole[0] = FALSE;
    p->contour[0].num_vertices = 3;
...
void gpc_write_polygon(FILE *fp, const int write_hole_flags, gpc_polygon *p) {

    [[maybe_unused]] int numprint = fprintf(fp, "%d\n", p->num_contours);
    for (int c = 0; c < p->num_contours; c++) {
        numprint = fprintf(fp, "%d\n", p->contour[c].num_vertices);

        if (write_hole_flags)
            numprint = fprintf(fp, "%d\n", p->hole[c]);

        for (int v = 0; v < p->contour[c].num_vertices; v++) {
            numprint = fprintf(fp, "% .*lf % .*lf\n", DBL_DIG, p->contour[c].vertex[v].x, DBL_DIG, p->contour[c].vertex[v].y);
        }
    }
}

void gpc_add_contour(gpc_polygon *p, const gpc_vertex_list *new_contour, const int hole) {
    int *extended_hole, c;
    ...
    for (int v = 0; v < new_contour->num_vertices; v++)
...
void gpc_polygon_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip, gpc_polygon *result) {
    ...
    polygon_node *out_poly = nullptr, *p, *q, *cf = nullptr;
    vertex_node *nv;
    h_state horiz[2];
...
void gpc_free_tristrip(gpc_tristrip *t) {
    ...
    for (int s = 0; s < t->num_strips; s++) FREE(t->strip[s].vertex)
    FREE(t->strip)
    t->num_strips = 0;
}
...
void gpc_tristrip_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip, gpc_tristrip *result) {
    ...
    it_node *it = nullptr, *intersect;
    edge_node *edge, *prev_edge, *next_edge, *succ_edge, *e0, *e1;
    edge_node *aet = nullptr, *c_heap = nullptr, *s_heap = nullptr, *cf = nullptr;
    lmt_node *lmt = nullptr, *local_min;
    polygon_node *tlist = nullptr, *p, *q;
    h_state horiz[2];
    vertex_type cft{};
    int in[2], exists[2], parity[2] = {LEFT, LEFT};
    int contributing, search, scanbeam = 0, sbt_entries = 0;
    int vclass, bl, br, tl, tr;
    double *sbt = nullptr, xb = 0, px = 0, nx = 0, yb = 0, yt = 0, dy = 0, ix = 0, iy = 0;
