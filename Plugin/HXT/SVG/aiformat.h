...
private:
    ...
    AIToken *prev{nullptr};
    AIToken *next{nullptr};
public:
    AITokenKind kind;

    AIToken(const AITokenKind k) : kind(k) { }
...
public:
    ...
    explicit AIComment(const LPCTSTR s) : AIToken(AIKComment), text(s) { }
...
public:
    // should all be AIPath objects
    AIObjectList members;
...
class AIParser {
    AITokenList tokens;
    AIToken *curtok{nullptr};
    AIPath *curpath{nullptr};
    AICompoundPath *curcpath{nullptr};
    BOOL instring{FALSE};
    BOOL inhexstring{FALSE};
    TCHAR *holdstring{nullptr};
    int holdlen{0};
    int holdmax{2000};
