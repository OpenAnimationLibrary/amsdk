...
PolygonalAreaArray *FindPolygonalAreas(Subdiv *s) {
    ...
    for (int i = 0; i < numareas; ++i) {
        ...
        if (pa->islands) {
            for (int j = 0; j < pa->islands->Num(); ++j)
...
static BOOL InsetUseTangents(const Vector &diff, const float inset) {
    ...
    return difflen == 0.0F || inset / difflen <= 0.4F;
}
...
static float BezArcLen(const Subdiv *s, const Edge *e) {
    ...
    if (e->IsCubic()) {
        ...
        return (len0 + len1 + len2 + len3) / 2.0F;
    }
    return len0;
}
...
static void PolyCutCorners(Subdiv *s, Edge *e, const float inset, const BOOL round) {
    ...
    for (; pit.Cur() != nullptr; pit.GetNext()) {
        ...
        if (fprev == nullptr)
            continue;
//        Vector prevpt = s->Coords(fprev->Org());
        Vector pt = s->Coords(f->Org());
//        Vector nextpt = s->Coords(f->Dst());
        ...
        const float alpha1 = 1.0F - inset / prevlen;
        ...
        if (round) {
            ...
            cp0 = LinInterp(1.0F / 3.0F, org, dst);
            cp1 = LinInterp(2.0F / 3.0F, org, dst);
        }
...
static float PolyFirstEdgeEvent(const Subdiv *subdiv, Edge *e, const BOOL inward) {
    ...
    for (; pit.Cur() != nullptr; pit.GetNext()) {
        ...
        const Vector pline = Cross(Vector(p.x, p.y, 1.0F), Vector(p.x + pdir.x, p.y + pdir.y, 1.0F));
        const Vector qline = Cross(Vector(q.x, q.y, 1.0F), Vector(q.x + qdir.x, q.y + qdir.y, 1.0F));
...
// If there is no such event, return -1.0f.
//
...
static float SplitEvent(const Vector &p, const Vector &pdir, const float sintheta, const Vector &q, const Vector &r, const BOOL inward) {
    if (sintheta < 1e-7F)
        return -1.0F; // angle at p is too cuspy; numeric error will be too great
    ...
    const Vector2 a = 1.0F / sintheta * n - m;
    if (a.x == 0.0F && a.y == 0.0F)
        return -1.0F; // m points in wrong direction (same as n)
    ...
    if (t < 0.0 || t > qr.Norm())
        return -1.0F; // s not between q and r
    float h;
            ...
            for (int i = istart; i < iend; ++i) {
                ...
                if (round && (i == 0 || i == n - 2)) {
                    ...
                    if (hdist != 0.0F && evec.z != 0.0F) {
                        ...
                        Vector cp1;
                        if (i == 0) {
                            cp0 = org + kappa * Vector(evec.x, evec.y, 0.0F);
                            cp1 = dst + kappa * Vector(0.0F, 0.0F, -evec.z);
                        } else {
                            cp0 = org + kappa * Vector(0.0F, 0.0F, evec.z);
                            cp1 = dst - kappa * Vector(evec.x, evec.y, 0.0F);
                        }
                        esp->SetBezCP(0, cp0);
                        esp->SetBezCP(1, cp1);
                    }
