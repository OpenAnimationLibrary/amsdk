...
class Edge {
    int num; // index of this in containing QuadEdge
    int val; // for user use
    float fval; // also for user use
    Edge *next; // Onext pointer

public:
    Edge(): num{0}, val{0}, fval{0}, next{nullptr} { }
    ...
    /*
        ...
        Edge *Onext(); // next ccw around origin ring
        ...
        Edge *Lprev(); // next cw around left face (if this is an "edge" edge)
...
class QuadEdge {
    Edge edges[4];
    QuadEdge *next; // in list of all QuadEdges of a subdivision
    int org; // index into subdivision's points of edge[0] origin
    int dst; // index into subdivision's points of edge[0] destination
    // (reverse them for edge[2])
    int spline; // id number for spline (chain of Edges) that edge[0] is
    // part of (positive if directed same as edge[0], negative
    // if directed opposite).
    Vector *bezcps; // if nonzero, then the edge is bezier cubic and
    // this points to array of two cps between org and dst
    unsigned long flags; // for user use
...
class Points {
    Vector *v; // array of points
    int numv; // number of valid entries in v
    int allocv; // current allocated size of v
    BOOL sorted; // true when v is sorted
...
public:
    EdgeList() : head(nullptr), tail(nullptr), num(0) { }
    ~EdgeList();
...
class Subdiv {
    QuadEdge *qedges; // all QuadEdges in subdivision (linked through next)
    Points *points; // vertex coords stored here
