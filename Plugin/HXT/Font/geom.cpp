...
static int LeftKid(const int i) {
    return 2 * i + 1;
}

static int RightKid(const int i) {
    return 2 * i + 2;
}

static void setbit(unsigned long &v, const int bit) {
    v |= 1U << bit;
}

static int isbitset(const unsigned long v, const int bit) {
    return v & 1U << bit;
}
...
float Angle(const Vector &a, const Vector &b) {
    ...
    if (norm1 != 0.0F && norm2 != 0.0F) {
        float costheta = Dot(a, b) / (norm1 * norm2);
        if (costheta > 1.0F)
            costheta = 1.0F;
        else if (costheta < -1.0F)
            costheta = -1.0F;
        ang = hash_math::acos(costheta) * 180.0 / hash_constants::double_pi;
    }
    return (float)ang;
}
...
void Subdiv::DeleteEdge(Edge *e) const {
    ...
    ASSERT(e->Onext() == e && e->Dnext() == e && e->Rot()->Onext() == e->InvRot() && e->InvRot()->Onext() == e->Rot());
...
BOOL Subdiv::PointInCircle(const int a, const int b, const int c, const int d) const {
    ...
    const double det = x[0] * (y[1] * z[2] - y[2] * z[1] - y[1] * z[3] + y[3] * z[1] + y[2] * z[3] - y[3] * z[2]) - x[1] * (y[0] * z[2] - y[2] * z[0] - y[0] * z[3] + y[3] * z[0] + y[2] * z[3] - y[3] * z[2]) + x[2] * (
                           y[0] * z[1] - y[1] * z[0] - y[0] * z[3] + y[3] * z[0] + y[1] * z[3] - y[3] * z[1]) - x[3] * (y[0] * z[1] - y[1] * z[0] - y[0] * z[2] + y[2] * z[0] + y[1] * z[2] - y[2] * z[1]);
...
float Subdiv::Angle(const int a, const int b, const int c) const {
    ...
    double ang = 0.0;
    if (norm1 != 0.0F && norm2 != 0.0F) {
        float costheta = Dot(u, v) / (norm1 * norm2);
        if (costheta > 1.0F)
            costheta = 1.0F;
        else if (costheta < -1.0F)
            costheta = -1.0F;
...
SubdivSplineIter::SubdivSplineIter(const Subdiv *subd) : cursp(0), subdiv(subd), cur(nullptr) {
...
SubdivSplineIter::~SubdivSplineIter() = default;
...
LeftFaceIter::LeftFaceIter(Edge *estart) {
    start = estart;
    cur = estart;
}
...
OnextIter::OnextIter(Edge *estart) {
    start = estart;
    cur = estart;
}
...
SplineIter::SplineIter(Edge *estart) {
    start = estart;
    cur = estart;
}
...
EdgeArray::EdgeArray(const EdgeArray &other) {
    if (other.num > 0) {
        ...
        memcpy(edges, other.edges, other.num * sizeof(Edge *));
    } else
        edges = nullptr;
    num = other.num;
    allocnum = other.num;
...
//            newallocn = 1;
        ...
        memcpy(newedges, this->edges, this->num * sizeof(Edge *));
...
void EdgeArray::Sort(const EdgeSorter sortfun) const {
    if (num > 1)
        qsort(edges, num, sizeof(Edge *), (int (_cdecl *)(const void *, const void *))sortfun);
}
...
static BOOL RecurInter(const Bezier2 *abez, const Bezier2 *bbez, unsigned long &asig, unsigned long &bsig, const int anode, const int bnode, int adepth, int bdepth) {
    ...
    if (adepth > 0) {
        ...
        if (bdepth > 0) {
            ...
            if (abezl.IntersectsBB(&bbezl))
                ans |= RecurInter(&abezl, &bbezl, asig, bsig, LeftKid(anode), LeftKid(bnode), adepth, bdepth);
            if (abezr.IntersectsBB(&bbezl))
                ans |= RecurInter(&abezr, &bbezl, asig, bsig, RightKid(anode), LeftKid(bnode), adepth, bdepth);
            if (abezl.IntersectsBB(&bbezr))
                ans |= RecurInter(&abezl, &bbezr, asig, bsig, LeftKid(anode), RightKid(bnode), adepth, bdepth);
            if (abezr.IntersectsBB(&bbezr))
                ans |= RecurInter(&abezr, &bbezr, asig, bsig, RightKid(anode), RightKid(bnode), adepth, bdepth);
        } else {
            if (abezl.IntersectsBB(bbez))
                ans |= RecurInter(&abezl, bbez, asig, bsig, LeftKid(anode), bnode, adepth, bdepth);
            if (abezr.IntersectsBB(bbez))
                ans |= RecurInter(&abezr, bbez, asig, bsig, RightKid(anode), bnode, adepth, bdepth);
        }
    } else if (bdepth > 0) {
        bbez->Subdivide(0.5F, &bbezl, &bbezr);
        setbit(bsig, bnode);
        ...
        if (abez->IntersectsBB(&bbezl))
            ans |= RecurInter(abez, &bbezl, asig, bsig, anode, LeftKid(bnode), adepth, bdepth);
        if (abez->IntersectsBB(&bbezr))
            ans |= RecurInter(abez, &bbezr, asig, bsig, anode, RightKid(bnode), adepth, bdepth);
    } else {
...
void gtrace(const char *fmt, ...) {
    ...
    if (gdebug & 2) {
        if (dbgf == nullptr)
            fopen_s(&dbgf, "E:\\am_debug\\font_plugin.txt", "w");
        if (dbgf != nullptr) {
