...
#ifdef _DEBUG
#define DATA_ACCESS_OPERATOR(var,i) ((*this)[i]) // for better bug tracking
#define FAST_ACCESS_OPERATOR(var,i) ((var)[(i)]) // for better bug tracking
#else
#define DATA_ACCESS_OPERATOR(var,i) ((var).m_pData[i]) // 10 times faster
...
template<class TYPE, class ARG_TYPE>
class CArrayEx final : public CArray<TYPE, ARG_TYPE> {
    // Construction
public:
    CArrayEx();
...
template<class TYPE, class ARG_TYPE>
CArrayEx<TYPE, ARG_TYPE> &CArrayEx<TYPE, ARG_TYPE>::operator =(const CArrayEx &x) {
    if (this != &x) {
        this->Copy(x);
    }
    return *this;
...
BOOL CArrayEx<TYPE, ARG_TYPE>::operator <(const CArrayEx &x) const {
    ...
    for (int i = 0; i < nSize; ++i) {
        if (!(DATA_ACCESS_OPERATOR(x, i) < FAST_ACCESS_OPERATOR(x, i))) {
            return FALSE;
        }
    }
