...
IEPolyModel *CHxtApp::ImportPLY(const String &pfilename) {
    int count;
    char name[3];
    FileStream fs;
...
void CHxtApp::LoadBones(FileStream &fs, IEBone **parentuplink, IEBone *parent) {
    ...
    while (sibling && !fs.Eof()) {
        ...
        if (f_version >= 3) {
            bone->m_weightarray.SetSize(count);
            for (int i = 0; i < count; ++i)
                fs >> bone->m_weightarray[i];
        }
        if (child)
