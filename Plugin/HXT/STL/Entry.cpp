...
enum { EXPORT_MODELCACHE, IMPORT_MODELCACHE, IMPORT_OBJECTCACHECONTAINER, EXPORT_ACTIONCACHE, EXPORT_CHOR };
...
extern "C" __declspec(dllexport) IEPolyModel *OnImportPropModel(const char *filename, const BOOL isnewimport, const float scale) {
    AFX_MANAGE_STATE(AfxGetStaticModuleState())
    return ((CHxtApp *)AfxGetApp())->ImportSTL(filename, isnewimport,TRUE);
}
