...
HFActionGetURL::HFActionGetURL(const char *_url) {
    url = _url;
    target = "";
}

HFActionGetURL::HFActionGetURL(const char *_url, const char *_target) {
    url = _url;
    target = _target;
}

void HFActionGetURL::AddToActionCondition(FActionCondition *ac) {
    ac->AddActionRecord(new FActionGetURL(new FString((U8 *)url), new FString((U8 *)target)));
}

void HFActionGetURL::AddToAction(FCTDoAction *doAction) {
    doAction->AddAction(new FActionGetURL(new FString((U8 *)url), new FString((U8 *)target)));
}
...
HFActionGoToFrame::HFActionGoToFrame(const int f) {
    frame = f;
}
