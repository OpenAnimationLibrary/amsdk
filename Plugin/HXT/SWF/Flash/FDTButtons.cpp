...
FButtonRecord1::FButtonRecord1(const U8 _hit, const U8 _down, const U8 _over, const U8 _up, const U16 _layer, FMatrix *_matrix) {
    hit = _hit;
    down = _down;
    over = _over;
    up = _up;
...
FButtonRecord2::FButtonRecord2(const U8 _hit, const U8 _down, const U8 _over, const U8 _up, const U16 _characterID, const U16 _layer, FMatrix *_matrix, FACXForm *_colorTransform) {
    hit = _hit;
    down = _down;
    over = _over;
    up = _up;
...
FButtonRecordList::FButtonRecordList() = default;
...
int FButtonRecordList::Size() const {
...
void FButtonRecordList::WriteToSWFStream(FSWFStream *_SWFStream) {
    ...
    for (std::list<FAButtonRecord *>::iterator cursor = listOfButtonRecords.begin(); cursor != listOfButtonRecords.end(); ++cursor) {
...
void FDTDefineButton::WriteToSWFStream(FSWFStream *_SWFStream) {
    ...
    for (std::list<FButtonRecord1 *>::iterator cursor = listOfButtonRecords.begin(); cursor != listOfButtonRecords.end(); ++cursor) {
    ...
    for (std::list<FActionRecord *>::iterator cursor2 = listOfActionRecords.begin(); cursor2 != listOfActionRecords.end(); ++cursor2) {
...
FDTDefineButton2::FDTDefineButton2(const U8 _menuFlag) {
...
FDTDefineButton2::~FDTDefineButton2() {
...
void FDTDefineButton2::WriteToSWFStream(FSWFStream *_SWFStream) {
    ...
    for (std::list<FButtonRecord2 *>::iterator cursor = listOfButtonRecords.begin(); cursor != listOfButtonRecords.end(); ++cursor) {
    ...
    const U32 offset = buttonRecordStream.Size() + 2; //have to count the action offset also
    ...
    std::list<FActionCondition *>::iterator cursor2 = conditionList.end();
    --cursor2;
...
FDTDefineButtonCXForm::FDTDefineButtonCXForm(const U16 _characterID, FCXForm *_colorTransform) {
    characterID = _characterID;
    colorTransform = _colorTransform;
}
...
FDTDefineButtonSound::FDTDefineButtonSound(const U16 _buttonID, const U16 _soundID0, FSoundInfo *_soundInfo0, const U16 _soundID1, FSoundInfo *_soundInfo1, const U16 _soundID2, FSoundInfo *_soundInfo2, const U16 _soundID3, FSoundInfo *_soundInfo3) {
    buttonID = _buttonID;
    soundID0 = _soundID0;
