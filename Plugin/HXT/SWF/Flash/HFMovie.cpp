...
HFFrame *HFMovie::Frame(const int index) {
    const auto current = frameVector.size();
    ...
    if (index >= (int)frameVector.size()) {
        frameVector.resize(index + 8); // give ourselves some extra space.

        for (auto i = current; i < frameVector.size(); i++) {
...
void HFMovie::WriteMovie(const char *filename) {
    ...
    for (std::map<int, std::list<AddRemoveEvent>>::iterator mapIt = arListMap.begin(); mapIt != arListMap.end(); ++mapIt) {
        arList.merge(mapIt->second);
    }

    FLASHOUTPUT("Depth sorting: %zd shapes\n", arList.size());
    ...
    for (int i = 0; i < framesInUse; i++) {
        HFFrame *frame = frameVector[i];
        std::list<FObj *> *objectList = frame->GetObjList();
    ...
    for (size_t i1 = 0; i1 < frameVector.size(); i1++) {
        delete frameVector[i1];
    }
