...
FCXForm::FCXForm(const U32 _hasAdd, const U32 _hasMult, const S32 _redMultTerm, const S32 _greenMultTerm, const S32 _blueMultiTerm, const S32 _redAddTerm, const S32 _greenAddTerm, const S32 _blueAddTerm) {
    hasAdd = _hasAdd;
    hasMult = _hasMult;
    redMultTerm = _redMultTerm;
    greenMultTerm = _greenMultTerm;
    blueMultTerm = _blueMultiTerm;
...
FCXFormWAlpha::FCXFormWAlpha(const U32 _hasAdd, const U32 _hasMult, const S32 _redMultTerm, const S32 _greenMultTerm, const S32 _blueMultTerm, const S32 _alphaMultTerm, const S32 _redAddTerm, const S32 _greenAddTerm, const S32 _blueAddTerm, const S32 _alphaAddTerm) {
    hasAdd = _hasAdd;
    hasMult = _hasMult;
...
U32 FCXFormWAlpha::MinBits() const {

    // FFileWrite's MaxNum method takes only 4 arguments, so finding maximum value of 8 arguments takes three steps:
    const U32 maxMult = FSWFStream::MaxNum(redMultTerm, greenMultTerm, blueMultTerm, alphaMultTerm);
    const U32 maxAdd = FSWFStream::MaxNum(redAddTerm, greenAddTerm, blueAddTerm, alphaAddTerm);
    const U32 maxValue = FSWFStream::MaxNum((S32)maxMult, (S32)maxAdd, 0, 0);
...
U32 FDTDefineMorphShape::AddFillStyle(FMorphFillStyle *fillStyle) const {
...
U32 FDTDefineMorphShape::AddLineStyle(const U32 startLineWidth, FColor *startLineColor, const U32 finalLineWidth, FColor *finalLineColor) const {
    assert(!styleArraysFinished);
    ...
    auto *startToFinalLine = new FMorphLineStyle(startLineWidth, finalLineWidth, startLineColor, finalLineColor);
    return morphLineStyleArray->Add(startToFinalLine);

}

void FDTDefineMorphShape::FinishStyleArrays() {
...
void FDTDefineShape::AddShapeRec(FShapeRec *_shapeRec) const {
...
U32 FDTDefineShape::AddFillStyle(FFillStyle *fillStyle) const {
...
// here changed.
U32 FDTDefineShape::AddSolidFillStyle(FColor *fillColor) const {
...
U32 FDTDefineShape::AddLineStyle(const U32 lineWidth, FColor *lineColor) const {

    assert(!styleArraysFinished); //once complete, cannot change 

    // here changed.
    lineColor->AlphaChannel(false);
    auto *lineStyle = new FLineStyle(lineWidth, lineColor);
...
*/
void FDTDefineShape::FinishStyleArrays() {

    styleArraysFinished = true;
    shapeWithStyle = new FShapeWStyle(fillStyleArray, lineStyleArray);
    fillStyleArray = nullptr; /*AMM*/
    lineStyleArray = nullptr;

}

U16 FDTDefineShape::ID() {
...
void FDTDefineShape2::AddShapeRec(FShapeRec *_shapeRec) const {
...
U32 FDTDefineShape2::AddFillStyle(FFillStyle *fillStyle) const {
...
// here changed.
U32 FDTDefineShape2::AddSolidFillStyle(FColor *fillColor) const {
    fillColor->AlphaChannel(false);
    FFillStyle *fillStyle = new FFillStyleSolid(fillColor);
...
U32 FDTDefineShape2::AddLineStyle(const U32 lineWidth, FColor *lineColor) const {
...
void FDTDefineShape2::FinishStyleArrays() {
...
void FDTDefineShape3::AddShapeRec(FShapeRec *_shapeRec) const {
...
U32 FDTDefineShape3::AddFillStyle(FFillStyle *fillStyle) const {
...
// here changed.
U32 FDTDefineShape3::AddSolidFillStyle(FColor *fillColor) const {
...
U32 FDTDefineShape3::AddLineStyle(const U32 lineWidth, FColor *lineColor) const {
...
void FDTDefineShape3::FinishStyleArrays() {

    styleArraysFinished = true;
    shapeWithStyle = new FShapeWStyle(fillStyleArray, lineStyleArray);
    fillStyleArray = nullptr;
    lineStyleArray = nullptr;

}

U16 FDTDefineShape3::ID() {
...
U32 FFillStyleArray::Size() const {
...
void FFillStyleArray::WriteToSWFStream(FSWFStream *_SWFStream) {
    ...
    //write the size
    const U32 size = fillStyleArray.size();
    ...
    for (std::list<FAFillStyle *>::iterator cursor = fillStyleArray.begin(); cursor != fillStyleArray.end(); ++cursor) {
...
//	clipped (tiledFlag==0).
FFillStyleBitmap::FFillStyleBitmap(const int tiled, const U16 ID, FMatrix *matrix) {
...
FFillStyleBitmap::~FFillStyleBitmap() {

    delete bitmapMatrix;
    bitmapMatrix = nullptr;
...
//	radial (linearFlag==0).
FFillStyleGradient::FFillStyleGradient(const int linear, FMatrix *matrix, FGradient *gradient) {
    linearFlag = linear;
...
FFillStyleGradient::~FFillStyleGradient() {
...
void FGradient::WriteToSWFStream(FSWFStream *_SWFStream) {
    ...
    for (std::list<FAGradRecord *>::iterator cursor = gradRecords.begin(); cursor != gradRecords.end(); ++cursor) {
...
FGradRecord::FGradRecord(const U32 _ratio, FColor *_color) {
    color = _color;
    ratio = _ratio;
}
...
FLineStyle::FLineStyle(const U32 _width, FColor *_color) {
    color = _color;
...
FLineStyleArray::FLineStyleArray() = default;
...
U32 FLineStyleArray::Size() const {
...
void FLineStyleArray::WriteToSWFStream(FSWFStream *_SWFStream) {
    ...
    //write the size
    const U32 size = lineStyleArray.size();
    ...
    for (std::list<FALineStyle *>::iterator cursor = lineStyleArray.begin(); cursor != lineStyleArray.end(); ++cursor) {
...
//	The tiledFlag indicates if the Bitmap fill style is tiled (tiledFlag==1) or 
//	clipped (tiledFlag==0).
FMorphFillStyleBitmap::FMorphFillStyleBitmap(const int tiled, const U16 ID, FMatrix *matrix1, FMatrix *matrix2) {
...
FMorphFillStyleBitmap::~FMorphFillStyleBitmap() {

    delete bitmapMatrix1;
    bitmapMatrix1 = nullptr;

    delete bitmapMatrix2;
    bitmapMatrix2 = nullptr;
...
//	The linearFlag indicates if the gradient fill style is linear (linearFlag==1) or 
//	radial (linearFlag==0).
FMorphFillStyleGradient::FMorphFillStyleGradient(const int linear, FMatrix *matrix1, FMatrix *matrix2, FGradient *gradient) {
...
FMorphFillStyleGradient::~FMorphFillStyleGradient() {
    ...
    gradFillMatrix1 = nullptr;
    delete gradFillMatrix2;
    gradFillMatrix2 = nullptr;
    delete gradFill;
    gradFill = nullptr;
...
//	Constructor.
FMorphGradRecord::FMorphGradRecord(const U32 _ratio1, FColor *_color1, const U32 _ratio2, FColor *_color2) {
...
FMorphLineStyle::FMorphLineStyle(const U32 _width1, const U32 _width2, FColor *_color1, FColor *_color2): color1(_color1), color2(_color2) {
    width1 = _width1;
    width2 = _width2;
...
void FShape::SetFillBits(const U32 _nFillBits) {
...
void FShape::SetLineBits(const U32 _nLineBits) {
...
void FShape::WriteToSWFStream(FSWFStream *_SWFStream) {
    ...
    for (std::list<FShapeRec *>::iterator cursor = shapeRecs.begin(); cursor != shapeRecs.end(); ++cursor) {
...
FShapeRecChange::FShapeRecChange(const U32 _stateNewStyles, const U32 _stateLineStyle, const U32 _stateFillStyle1, const U32 _stateFillStyle0, const U32 _stateMoveTo, const S32 _moveDeltaX, const S32 _moveDeltaY, const U32 _fill0Style, const U32 _fill1Style, const U32 _lineStyle, FFillStyleArray *_fillStyles, FLineStyleArray *_lineStyles) {
...
FShapeRecChange::~FShapeRecChange() {

    if (fillStyles) {
        //if fillStyles isn't NULL
...
*/
void FShapeRecChange::IncludeNFillBitInfo(const U32 _nFillBits) {
...
*/
void FShapeRecChange::IncludeNLineBitInfo(const U32 _nLineBits) {
...
U32 FShapeRecChange::MinBits() const {

    const U32 max = FSWFStream::MaxNum(moveDeltaX, moveDeltaY, 0, 0);
...
FShapeRecEdgeStraight::FShapeRecEdgeStraight(const S32 dx, const S32 dy) {
    generalLineFlag = 0;
    generalDeltaX = 0;
    generalDeltaY = 0;
    verticalLineFlag = 0;
...
U32 FShapeRecEdgeStraight::MinBits() const {

    const U32 maxDelta = FSWFStream::MaxNum(generalDeltaX, generalDeltaY, horizontalDeltaX, verticalDeltaY);
...
FShapeRecEdgeCurved::FShapeRecEdgeCurved(const S32 controlDX, const S32 controlDY, const S32 anchorDX, const S32 anchorDY) {
    edgeFlag = 0;
...
U32 FShapeRecEdgeCurved::MinBits() const {

    const U32 maxDelta = FSWFStream::MaxNum(controlDeltaX, controlDeltaY, anchorDeltaX, anchorDeltaY);
...
FShapeRecEnd::FShapeRecEnd() = default;
...
U32 FShapeWStyle::NumFillBits() const {

    return nFillBits;

}

U32 FShapeWStyle::NumLineBits() const {
