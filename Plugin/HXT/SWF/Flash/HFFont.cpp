...
void HFFont::AddGlyph(HFPolygon *poly, const int unicode, const int advance) {
    Glyph glyph{};
...
void HFFont::AddToObjectList(std::list<FObj *> *objList, HFMovie *movie) {
    ...
    if (!defineWritten) {
        ...
        for (std::list<Glyph>::iterator it = glyphList.begin(); it != glyphList.end(); ++it) {
...
int HFFont::GetGlyphCodeFromAscii(const int unicode) {
    int code = 0;

    for (std::list<Glyph>::iterator it = glyphList.begin(); it != glyphList.end(); ++it, ++code) {
        if (it->unicode == unicode) {
            FLASHASSERT(code < (int)glyphList.size());
            FLASHASSERT(code >= 0);
            return code;
        }
    }
...
int HFFont::GetAdvance(int glyphCode) {
    ...
    FLASHASSERT(glyphCode < (int)glyphList.size());
