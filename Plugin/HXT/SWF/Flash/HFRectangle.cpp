...
void HFRectangle::AddToObjectList(std::list<FObj *> *objList, HFMovie *movie) {
    if (!defineWritten) {
        ...
        rectangleShapeRecords[0] = new FShapeRecChange(false, true, true, false, true, bounds.Xmin(), bounds.Ymin(), 0, colorFillID, lineStyleID, nullptr, nullptr);
        ...
        for (auto &rectangleShapeRecord: rectangleShapeRecords)
            shapeTag->AddShapeRec(rectangleShapeRecord);
