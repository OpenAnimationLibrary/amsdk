...
FDTDefineSoundADPCM::FDTDefineSoundADPCM(const U8 rate, const U8 size, const U8 type, const U32 sampleCount, const U8 *data, const U32 dataSize, const int compression) {
    ...
    sound.samples = const_cast<U8 *>(data); // it won't change it - so we cast (lee)
    sound.dataLen = dataSize;
    sound.delay = 0;

    FSoundComp compress(&sound, compression);

    compress.Compress(const_cast<U8 *>(data), sound.nSamples, &pcmData); // it won't change it - so we cast (lee)
...
FDTDefineSoundWAV::FDTDefineSoundWAV(FILE *wavFile, const int compression) {
    ...
    if (!loadWavFile(wavFile, &soundData, &wavData, &wavDataSize)) {
        // there was an error
        return;
    }
...
bool FDTDefineSoundWAV::loadWavFile(FILE *fp, SSound *soundData, U8 **wavData, int *wavDataSize) {
    ...
    const unsigned long fileTag = read32(fp);
    if (fileTag != 0x46464952) {
        FLASHOUTPUT("Not a RIFF format file\n");
        return false;
    }

    unsigned long filesize = read32(fp);
    filesize += ftell(fp);

    const unsigned long typeTag = read32(fp);
    if (typeTag != 0x45564157) {
        FLASHOUTPUT("Not a WAVE file\n");
        return false;
    }

    unsigned long nextseek = ftell(fp);
    while (nextseek < filesize) {
        fseek(fp, nextseek, 0);
        const unsigned long chunkId = read32(fp);
        const unsigned long chunkSize = read32(fp);
        ...
        switch (chunkId) {
            case 0x20746D66: //Format Chunk
            {
                ...
                switch (formatTag) {
                    default:case 0x0000:FLASHOUTPUT("WAVE_FORMAT_UNKNOWN");
                        break;
                    ...
                    case 0x0007:FLASHOUTPUT("WAVE_FORMAT_MULAW\n");
                        break;
                    case 0x0010:FLASHOUTPUT("WAVE_FORMAT_OKI_ADPCM");
                        break;
                ...
                const int channels = read16(fp);
                soundData->type = channels - 1;
                FLASHOUTPUT("  %u Channels\n", channels);

                const int samplesPerSec = read32(fp);
                ...
                switch (samplesPerSec / 5000) {
                    ...
                    case 8: // 44k
                        ...
                        break;
                    default:FLASHOUTPUT("Sample rate %d unsupported\n", samplesPerSec);
                        return false;
                }

                const int bytesPerSec = read32(fp);
                FLASHOUTPUT("  %u Average Bytes Per Second\n", bytesPerSec);

                const int blockAlign = read16(fp);
                FLASHOUTPUT("  Block Align %u\n", blockAlign);

                FLASHOUTPUT("Format Specific Fields:\n");
                const int bits = read16(fp);
            ...
            case 0x61746164: //Data Chunk
            {
                ...
                *wavData = new U8[chunkSize];
                const int read = fread(*wavData, 1, *wavDataSize, fp);
            ...
            default:FLASHOUTPUT("Unknown Chunk\n");
                return false;
        }
    }
    return false;
...
FDTSoundStreamBlock::FDTSoundStreamBlock(const U32 _size, U8 *_data) {
    size = _size;
    data = _data;
}
...
FDTSoundStreamHead::FDTSoundStreamHead(const U8 _mixFormat, const U8 _soundType, const U16 _count) {
...
FDTSoundStreamHead2::FDTSoundStreamHead2(const U8 _mixFormat, const U8 _compression, const U8 _size, const U8 _soundType, const U16 _count) {
    mixFormat = _mixFormat;
...
void FSndEnv::WriteToSWFStream(FSWFStream *_SWFStream) const {
...
//////////////////////////////////////////////////////////////////////////////////////
//  --------  FSoundInfo  ------------------------------------------------------------
FSoundInfo::FSoundInfo(const U8 _syncFlags, const U8 _hasEnvelope, const U8 _hasLoops, const U8 _hasOutPoint, const U8 _hasInPoint, const U32 _inPoint, const U32 _outPoint, const U16 _loopCount, const U8 _nPoints, FSndEnv *_soundEnvelope) {
...
void FSoundInfo::WriteToSWFStream(FSWFStream *_SWFStream) const {
