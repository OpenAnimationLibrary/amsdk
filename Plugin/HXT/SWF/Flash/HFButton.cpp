...
HFButton::HFButton(HFShape *shape1, HFShape *shape2, HFShape *shape3, HFShape *shape4) {
    shape[0] = shape1;
    shape[1] = shape2;
    shape[2] = shape3;
    shape[3] = shape4;
...
void HFButton::QueueEvent(HFAction *action, const int event) const {
    AddEvent(event);
...
void HFButton::AddToObjectList(std::list<FObj *> *objList, HFMovie *movie) {
    ...
    if (!defineWritten) // only write the define tag once!
    {
        ...
        auto *buttonTag = new FDTDefineButton2(0); //0 flags a push button
        ...
        for (i = 0; i < 4; i++) {
            ...
            auto *bRec = new FButtonRecord2(i == HIT,
                                            i == DOWN,
                                            i == OVER,
                                            i == UP, // 
                                            shape[i]->DefineTagId(),
...
void HFButton::AddEvent(const int event) const {
    switch (event) {
        case MENU_DRAG_OUT:
