...
void CreateBitsMovie() {
    ...
    auto *background = new FCTSetBackgroundColor(new FColor(red));
    ...
    auto *bits = new FDTDefineBitsLossless(FDTDefineBitsLossless::bm8Bit, // format	
                                           BITMAP_X, // width
                                           BITMAP_Y, // height
    ...
    auto *rectBounds = new FRect(0, 0, 1 * SCoord1, 1 * SCoord1); // Coordinate values are in TWIPS - it
    ...
    auto *matrix1 = new FMatrix(true, // Has scale terms.
    ...
    const U32 fillStyle1_ID = rectangle->AddFillStyle(fill1); // fillStyle1_ID is 2

    // Add black border of 200 twips to line style array.
    const U32 blackLineStyleID = rectangle->AddLineStyle(200, new FColor(black));
