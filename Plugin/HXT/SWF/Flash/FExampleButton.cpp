...
void MakeButtonMovie() {
    ...
    auto *doAction1 = new FCTDoAction();
    ...
    //remember ID
    const U16 buttonUpID = CreateRectangle(&allTags, new FColor(red), -1159, 1260, -620, 560);

    //construct a DTDefineShape object which will be the button's over image
    const U16 buttonOverID = CreateRectangle(&allTags, new FColor(green), -1159, 1260, -620, 560);

    //construct a DTDefineShape object which will be the button's down image
    const U16 buttonDownID = CreateRectangle(&allTags, new FColor(blue), -1159, 1260, -620, 560);

    //construct a DTDefineShape object which will be the button's hit image
    const U16 buttonHitID = CreateRectangle(&allTags, new FColor(black), -1159, 1260, -620, 560);

    //now define the actual button
    auto *theButton = new FDTDefineButton2(0); //flag is 0 to indicate a push button
    const U16 theButtonID = theButton->ID();

    //button up record, defines how the button will look like in the up position

    auto *mx = new FMatrix(); //blank matrix(by default identity)
    auto *cxf = new FCXFormWAlpha(false, false, 0, 0, 0, 0, 0, 0, 0, 0);
    auto *bRec = new FButtonRecord2(false, false, false, true, buttonUpID, 1, mx, cxf);
    ...
    auto *matrix = new FMatrix();

    //make a "place object tag", include identity matrix
    auto *placeButton = new FCTPlaceObject2(false, // ~ _hasClipDepth
                                            true,
                                            true,
                                            false,
                                            1,
    ...
    auto *remObj = new FCTRemoveObject2(1); //remove layer 1
    ...
    //construct an empty CTDoAction tag object
    auto *doAction2 = new FCTDoAction();
    ...
    const U16 rectangleID = CreateRectangle(&allTags, new FColor(white), 2020, 4900, 1680, 4000);
...
U16 CreateRectangle(FObjCollection *objCollection, FColor *fillColor, const SCOORD xmin, const SCOORD xmax, const SCOORD ymin, const SCOORD ymax) {

    //create a rectangle

    //construct a rect that defines the shape's bounds 
    auto *rectBounds = new FRect(xmin - 10, ymin - 10, xmax + 10, ymax + 10); //coordinate values are in TWIPS
    ...
    const U32 fillID = rectangle->AddSolidFillStyle(fillColor);
    ...
    const U32 blackLineStyleID = rectangle->AddLineStyle(20, new FColor(0, 0, 0));
