...
void HFFrame::SetBackgoundColor(const FRGBA color_RGBA) {
    // Convert RGBA to RGB. Background color really doesn't need alpha channel.
    auto *color_RGB = new FColor(color_RGBA.red, color_RGBA.green, color_RGBA.blue);
    auto *background = new FCTSetBackgroundColor(color_RGB);

    objList.push_back(background);
}

void HFFrame::AddAction(HFAction *action) {
    auto *doAction = new FCTDoAction();
