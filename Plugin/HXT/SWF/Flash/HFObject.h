...
struct AddRemoveEvent {
    ...
    bool operator==(const AddRemoveEvent &rhs) const {
        return originalDepth == rhs.originalDepth;
    }
...
public:
    ...
    virtual void RemoveFromFrame(std::list<FObj *> *objList, HFMovie *movie);

    [[nodiscard]] int DefineTagId() const {
        FLASHASSERT(objectID >= 0);
        return objectID;
    }
    ...
    void ObjectIsEmbedded(const bool _embedded) {
        embedded = _embedded;
    }
...
protected:
    ...
    void SetDefineTagId(const int id) {
        objectID = id;
    }
