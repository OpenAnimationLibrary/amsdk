...
void CreateCircleMovie() {
    ...
    auto *rectBounds = new FRect(0, 0, 2006, 2005); //coordinate values are in TWIPS
    ...
    const U32 blackLineStyleID = circle->AddLineStyle(20, new FColor(black));
    ...
    for (auto &circleShapeRecord: circleShapeRecords)
        circle->AddShapeRec(circleShapeRecord);
    ...
    auto *placeCircle = new FCTPlaceObject2(false, // ~ _hasClipDepth
                                            false,
                                            true,
                                            false,
                                            1,
