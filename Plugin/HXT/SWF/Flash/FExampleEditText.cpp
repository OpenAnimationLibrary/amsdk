...
void CreateTextFieldMovie() {
    ...
    auto *font = new FDTDefineFont2(fontname, FDT::ANSI, false, false);

    allTags.AddFObj(font);

    // Construct the bounding square.
    auto *rectBounds = new FRect(1000, 1000, 2000, 2000);
    // Construct black color for text.
    auto *black = new FColor(0, 0, 0, 255);
    // Variable name for text field.
    auto *var_name = new FString((U8 *)"TextField1");
    // Initial text string int the text field. 
    auto *init = new FString((U8 *)"start");

    // Construct a EditText SWF tag.
    auto *textField = new FDTDefineEditText(rectBounds,
                                            true, // _hasFont
                                            false, // _hasMaxLength
                                            true, // _hasTextColor
                                            false, // _readOnly
                                            ...
                                            init); // _initialText
    const U16 textFieldID = textField->ID(); // remember this tag's ID.
    allTags.AddFObj(textField); // Add the text field to the moive's character list.

    // Now place the text field onto the frame.
    auto *placeTextField = new FCTPlaceObject2(false, // ~ _hasClipDepth
                                               false,
                                               true,
                                               false,
                                               1,
