...
//////////////////////////////////////////////////////////////////////////////////////
//  --------  HFSound  ---------------------------------------------------------------
HFText::HFText(const char *_text, HFFont *_font) : HFShape(0, 0, 0, 0) {
    text = _text;
    font = _font;
...
void HFText::AddToObjectList(std::list<FObj *> *objList, HFMovie *movie) {
    ...
    if (!defineWritten) {
        constexpr int fontHeight = 1024;
        // create tag that defines the text
        auto *defineText = new FDTDefineText2(new FRect(bounds), new FMatrix(), font->GlyphCount());
        ...
        defineText->AddTextRecord(new FTextChangeRecord(true, true, false, false, font->DefineTagId(), fontHeight, new FColor(solidFillColor), 0, 0));
        ...
        for (size_t i = 0; i < text.size(); i++) {
            ...
            const int code = font->GetGlyphCodeFromAscii(c);
...
HFEditText::HFEditText(int xmin, int ymin, int xmax, int ymax, const char *_text) : HFShape(xmin, ymin, xmax, ymax) {
...
void HFEditText::SetEditable(const bool edit) {
    canEdit = edit;
}

void HFEditText::SetHeight(const int heightInTwips) {
...
void HFEditText::AddToObjectList(std::list<FObj *> *objList, HFMovie *movie) {
    ...
    [[maybe_unused]] auto nums = sprintf_s(buffer, 64, "TextField_%ld_%ld_%ld_%ld_%d", bounds.Xmin(), bounds.Ymin(), bounds.Xmax(), bounds.Ymax(), depth);
    ASSERT(nums < 65);

    auto *variable = new FString((U8 *)buffer);

    auto *textField = new FDTDefineEditText(new FRect(bounds),
                                            false, // _hasFont
                                            false, // _hasMaxLength
                                            true, // _hasTextColor
                                            !canEdit, // _readOnly
