...
void FObjCollection::WriteToSWFStream(FSWFStream *_SWFStream) {
    ...
    for (std::list<FObj *>::iterator cursor = FObjList.begin(); cursor != FObjList.end(); ++cursor) {
...
void FObjCollection::CreateMovie(const char *fileName, const SCOORD _movieWidth, const SCOORD _movieHeight, const U32 _frameRate) {
    FILE *fp;
...
void FObjCollection::CreateMovie(FSWFStream *swfStream, const SCOORD _movieWidth, const SCOORD _movieHeight, const U32 _frameRate) {
...
void FObjCollection::WriteFileHeader(U8 *target, const U32 _fileLengthNoHeader, const SCOORD _movieWidth, const SCOORD _movieHeight, const U32 _frameRate) const {
    FSWFStream header;
...
void FObjCollection::WriteEndTag(FSWFStream *_SWFStream) {
    ...
    _SWFStream->AppendTag(stagEnd, 0, nullptr);
}
...
FFragment::FFragment(const void *_data, const int _size) {
    data = _data;
    size = _size;
}
