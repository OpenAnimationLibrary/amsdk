...
void HFObject::AddPlaceObject2(std::list<FObj *> *objList, HFMovie *movie) {
    ...
    auto *placeObject = new FCTPlaceObject2(false, // has clip depth
                                            false, // has ratio
                                            true, // has char id
                                            false, // has move
                                            GetDepth(),
...
void HFObject::RemoveFromFrame(std::list<FObj *> *objList, HFMovie *movie) {
    auto *removeObject = new FCTRemoveObject(DefineTagId(), GetDepth());
    ...
    FLASHASSERT(ar.remove == nullptr); // doubly removed. Or there is an error with the id's.
