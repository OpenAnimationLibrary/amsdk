...
HFPolygon::HFPolygon(SPOINT *_pointArray, int _numberOfPoints) : HFShape(0, 0, 0, 0) {
    ...
    // Do we need to close?
    if (_pointArray[0].x == _pointArray[_numberOfPoints - 1].x && _pointArray[0].y == _pointArray[_numberOfPoints - 1].y) {
        // we are closed
...
HFPolygon::~HFPolygon() = default;

void HFPolygon::CalculateAndSetBounds() {
    int xmin, ymin;

    // Calculate the bounds - the min's and max's
    int x = xmin = pointArray[0].x;
    int xmax = pointArray[0].x;
    int y = ymin = pointArray[0].y;
    int ymax = pointArray[0].y;
...
void HFPolygon::AddStraightLine(const int dx, const int dy) {
    // Get enough memory
    if ((int)pointArray.size() <= numberOfPoints) {
...
void HFPolygon::AddCurvedLine(const S32 controlDX, const S32 controlDY, const S32 anchorDX, const S32 anchorDY) {
    // Get enough memory
    if ((int)pointArray.size() <= numberOfPoints) {
        pointArray.resize((numberOfPoints / ALLOCATE + 1) * ALLOCATE);
    }
...
void HFPolygon::AddToObjectList(std::list<FObj *> *objList, HFMovie *movie) {
    if (!defineWritten) {
        auto *rect = new FRect(bounds);
        auto *shapeTag = new FDTDefineShape3(rect);
        // 	objectID = shapeTag->ID();
        SetDefineTagId(shapeTag->ID());

        FillShapeHeader(objList, shapeTag);

        shapeTag->AddShapeRec(new FShapeRecChange(false, true, true, false, true, pointArray[0].x, pointArray[0].y, 0, colorFillID, lineStyleID, nullptr, nullptr));

        for (int i = 1; i < numberOfPoints; i++) {
            if (pointArray[i].straight) {
                shapeTag->AddShapeRec(new FShapeRecEdgeStraight(pointArray[i].x, pointArray[i].y));
            } else {
                shapeTag->AddShapeRec(new FShapeRecEdgeCurved(pointArray[i].controlDx, pointArray[i].controlDy, pointArray[i].anchorDx, pointArray[i].anchorDy));
            }
        }
...
FShape *HFPolygon::CreateFShape() const {
    auto *shape = new FShape();

    shape->AddShapeRec(new FShapeRecChange(false, false, true, false, true, pointArray[0].x, pointArray[0].y, 0, 1, 0, nullptr, nullptr));
    ...
    for (int i = 1; i < numberOfPoints; i++) {
        if (pointArray[i].straight) {
            shape->AddShapeRec(new FShapeRecEdgeStraight(pointArray[i].x, pointArray[i].y));
        } else {
            shape->AddShapeRec(new FShapeRecEdgeCurved(pointArray[i].controlDx, pointArray[i].controlDy, pointArray[i].anchorDx, pointArray[i].anchorDy));
        }
    }
