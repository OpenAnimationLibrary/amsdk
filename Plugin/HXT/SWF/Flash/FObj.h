...
class FObj {
public:
    virtual ~FObj() = default; // Doesn't do anything, just makes all the other destructors virtual
...
private:
    ...
    void WriteFileHeader(U8 *target, U32 _fileLengthNoHeader, SCOORD _movieWidth, SCOORD _movieHeight, U32 _frameRate) const;
    static void WriteEndTag(FSWFStream *_SWFStream);
