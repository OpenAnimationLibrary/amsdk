...
HFOval::HFOval(int _xMin, int _yMin, int _xMax, int _yMax) : HFShape(_xMin, _yMin, _xMax, _yMax) {
    xmin = _xMin;
    ymin = _yMin;
    xmax = _xMax;
    ymax = _yMax;
...
void HFOval::AddToObjectList(std::list<FObj *> *objList, HFMovie *movie) {
    if (!defineWritten) {
        ...
        const int dx = xmax - xmin; // dx is width diameter
        const int dy = ymax - ymin; // dy is height diameter
        const double begin = 0.8535; // starting position to draw (bounding square is 1x1)
        const int startX = int(xmin + begin * dx); // X coordinate of the starting position in twips
        const int startY = int(ymin + begin * dy); // Y coordinate of the starting position in twips

        // connect a serie of curves to draw the circle
        shapeTag->AddShapeRec(new FShapeRecChange(false, true, true, false, true, startX, startY, 0, colorFillID, lineStyleID, nullptr, nullptr));
...
HFCircle::HFCircle(int center_x, int center_y, int radius) : HFOval(center_x - radius, center_y - radius, center_x + radius, center_y + radius) {
...
HFCircle::HFCircle(int _xMin, int _yMin, int _xMax, int _yMax) : HFOval(_xMin, _yMin, _xMax, _yMax) {
    xmin = _xMin;
    ymin = _yMin;
    xmax = _xMax;
    ymax = _yMax;
