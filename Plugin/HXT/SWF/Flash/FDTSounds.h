...
public:
    ...
    FDTDefineSound();
    ~FDTDefineSound() override = default;
    ...
    [[nodiscard]] int NumSamples() const {
        return soundData.sampleCount;
    }

    [[nodiscard]] int Format() const {
        return 4 * soundData.rate + soundData.size * 2 + soundData.type;
    }

    [[nodiscard]] int SoundType() const {
        return soundData.type;
    }
...
class FDTDefineSoundADPCM : public FDTDefineSound {
public:
    FDTDefineSoundADPCM(U8 rate, U8 size, U8 type, U32 sampleCount, const U8 *data, U32 dataSize, int compression);
};
...
private:
    bool loadWavFile(FILE *fp, SSound *soundData, U8 **wavData, int *wavDataSize);

    static U32 read32(FILE *fp) {
        U32 val;
        fread(&val, 1, 4, fp);
        return val;
    }

    static U16 read16(FILE *fp) {
        U16 val;
        fread(&val, 1, 2, fp);
        return val;
    }
...
public:
    FSoundInfo(U8 _syncFlags, U8 _hasEnvelope, U8 _hasLoops, U8 _hasOutPoint, U8 _hasInPoint, U32 _inPoint, U32 _outPoint, U16 _loopCount, U8 _nPoints, FSndEnv *_soundEnvelope);
    ~FSoundInfo();
    void WriteToSWFStream(FSWFStream *_SWFStream) const;
