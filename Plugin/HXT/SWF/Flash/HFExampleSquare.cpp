...
void HLCreateSquareMovie() {
    ...
    constexpr int BITMAP_X = 128; // The size of the test bitmap.
    constexpr int BITMAP_Y = 128;

    // Load in a JPeg
    FILE *fp;
    fopen_s(&fp, "bm128.jpg", "rb");
    FLASHASSERT(fp);

    // get the file size
    fseek(fp, 0, SEEK_END);
    const int jpegSize = ftell(fp);
    fseek(fp, 0, SEEK_SET);
    ...
    HFBitmap *bitmapShape2 = new HFBitmap(300 * FPIXEL, 200 * FPIXEL, 500 * FPIXEL, 400 * FPIXEL, FDTDefineBitsLossless::bm8Bit, BITMAP_X, BITMAP_Y, SIZE_PALETTE, false, bitmapLossless, rgb);
    // EditText square
    HFEditText *editText = new HFEditText(50 * FPIXEL, 50 * FPIXEL, 250 * FPIXEL, 250 * FPIXEL, "This is an EditText object.");

    // Polygon
    SPOINT triangleData[] = {{25 * FPIXEL, 75 * FPIXEL}, {75 * FPIXEL, 75 * FPIXEL}, {50 * FPIXEL, 25 * FPIXEL}};
    auto *triangle = new HFPolygon(triangleData, 3);

    // Polygon
    SPOINT pentagonData[] = {{150 * FPIXEL, 75 * FPIXEL}, {200 * FPIXEL, 100 * FPIXEL}, {175 * FPIXEL, 125 * FPIXEL}, {125 * FPIXEL, 125 * FPIXEL}, {100 * FPIXEL, 100 * FPIXEL}};
    auto *pentagon = new HFPolygon(pentagonData, 5);
