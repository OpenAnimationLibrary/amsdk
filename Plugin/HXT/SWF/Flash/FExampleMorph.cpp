...
void MakeMorphMovie() {
    ...
    auto *background = new FCTSetBackgroundColor(new FColor(lightBlue));

    // Object is added to collection
    flashObjects.AddFObj(background);

    auto *morphingTriangle = new FDTDefineMorphShape(NewRect(2000, 2000, 8000, 8000), NewRect(2000, 2000, 8000, 8000));

    // Record morphing triangle's ID for later reference
    const U32 triangleID = morphingTriangle->ID();
    ...
    // reference
    const U32 blackToWhiteLineIndex = morphingTriangle->AddLineStyle(20, new FColor(blackA), 500, new FColor(whiteA));
    ...
    morphingTriangle->AddShapeRec_1(NewShapeRecChange(false, true, false, false, true, 5000, 4000, 0, 0, blackToWhiteLineIndex, nullptr, nullptr));
    ...
    morphingTriangle->AddEdgeRec_2(NewShapeRecChange(false, false, false, false, true, 5000, 4000, 0, 0, 0, nullptr, nullptr));
    ...
    auto *placeTriangleMorph = new FCTPlaceObject2(false, // ~ _hasClipDepth
                                                   false, //   has ratio
                                                   true, //	has ID		 
                                                   false, //	has move
                                                   1, // depth
...
FShapeRecChange *NewShapeRecChange(const U32 _newStylesFlag, const U32 _lineStyleFlag, const U32 _fillStyle0Flag, const U32 _fillStyle1Flag, const U32 _moveFlag, const S32 _deltaX, const S32 _deltaY, const U32 _fillStyle1, const U32 _fillStyle0, const U32 _lineStyle, FFillStyleArray *_fillStyleArray, FLineStyleArray *_lineStyleArray) {

    return new FShapeRecChange(_newStylesFlag, _lineStyleFlag, _fillStyle1Flag, _fillStyle0Flag, _moveFlag, _deltaX, _deltaY, _fillStyle1, _fillStyle0, _lineStyle, _fillStyleArray, _lineStyleArray);
}
...
FRect *NewRect(const S32 _xmin, const S32 _ymin, const S32 _xmax, const S32 _ymax) {
