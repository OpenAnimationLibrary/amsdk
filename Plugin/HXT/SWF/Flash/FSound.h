...
public:
    ...
    [[nodiscard]] S32 Rate() const {
        return kRateTable[format >> 2 & 0x3];
    }

    [[nodiscard]] int RateShift() const {
    ...
    [[nodiscard]] int CompressFormat() const {
    ...
    [[nodiscard]] S32 GetDuration44() const {
        return nSamples << RateShift();
    }

    void SetDuration44(const S32 d) {
        nSamples = d >> RateShift();
    }

    [[nodiscard]] int BytesPerBlock() const {
        return BytesPerSample() * NChannels();
    }
...
private:
    ...
    void PutBits(const S32 v, const int n, std::vector<U8> *stream) {
        assert(n<=16);
        if (bitPos + n > 32)
            WriteBits(stream);
