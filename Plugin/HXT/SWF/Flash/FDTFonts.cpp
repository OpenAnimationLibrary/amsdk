...
void FDTDefineFont::WriteToSWFStream(FSWFStream *_SWFStream) {
    ...
    const U32 offsetsBufferSize = shapeGlyphs.size() * 2;
...
FDTDefineFont2::FDTDefineFont2(const char *_fontName, const U16 _encodeType, const U16 _italicFlag, const U16 _boldFlag) {
...
FDTDefineFont2::FDTDefineFont2(const char *_fontName, const U16 _encodeType, const U16 _italicFlag, const U16 _boldFlag, const S16 _ascenderHeight, const S16 _descenderHeight, const S16 _leadingHeight) {
...
void FDTDefineFont2::AddShapeGlyph(FShape *_shape, const U16 _shapeCode, const S16 _shapeAdvance, FRect *_shapeBounds) {
...
U16 FDTDefineFont2::nIndexBits() const {

    return (U16)FSWFStream::MinBits(glyphs.size() - 1, false);

}

U16 FDTDefineFont2::ID() {
...
void FDTDefineFont2::WriteToSWFStream(FSWFStream *_SWFStream) {
    ...
    if (hasLayoutFlag) {
        ...
        for (std::list<FKerningRec *>::iterator kernCursor = kerningTable.begin(); kernCursor != kerningTable.end(); ++kernCursor) {
...
FDTDefineFontInfo::FDTDefineFontInfo(const char *_fontName, const U16 _fontID, const U16 _encodeType, const U16 _italicFlag, const U16 _boldFlag) {
...
void FDTDefineFontInfo::AddCode(const U16 _someCode) {
...
void FDTDefineFontInfo::WriteToSWFStream(FSWFStream *_SWFStream) {
    ...
    for (std::list<U16>::iterator cursor = codeTable.begin(); cursor != codeTable.end() && wideCodesFlag == 0; ++cursor) {
        if (*cursor > 65530)
            wideCodesFlag = 1;
    }
...
FKerningRec::FKerningRec(const U16 _code1, const U16 _code2, const short _kerningAdjust) {

    wideCodesFlag = 0; // default not wide
    code1 = _code1;
    code2 = _code2;
    kerningAdjust = _kerningAdjust;
}

void FKerningRec::CodesWide(const U16 _flag) {
