...
void CreateClipMovie() {
    ...
    auto *clipRectBounds = new FRect(500, 500, 4500, 4500); //coordinate values are in TWIPS

    //construct the FDTDefineShape which will be the clipRectangle image
    auto *clipRectangle = new FDTDefineShape(clipRectBounds);

    //record its ID so that we can later refer to it
    const U16 clipRectangleID = clipRectangle->ID();
    ...
    const U32 blackfillID = clipRectangle->AddSolidFillStyle(new FColor(black));
    ...
    ***/
    ...
    clipRectangleShapeRecords[0] = new FShapeRecChange(false, true, true, false, true, 4500, 500, 0, blackfillID, 0, nullptr, nullptr);
    ...
    auto *placeClipRectangle = new FCTPlaceObject2(true, false, true, false, 1, clipRectangleID, nullptr, nullptr, 0, nullptr, 3);
    ...
    auto *rectBounds = new FRect(1000, 1000, 5000, 5000); //coordinate values are in TWIPS
    ...
    rectangleShapeRecords[0] = new FShapeRecChange(false, true, true, false, true, 5000, 1000, 0, redfillID, blackLineStyleID, nullptr, nullptr);
    //Create straight edge object (just a stuct of info), store it in EdgeRecord
    ...
    auto *placeRectangle = new FCTPlaceObject2(false, false, true, false, 2, rectangleID, nullptr, nullptr, 0, nullptr, 0);
    ...
    auto *circle = new FDTDefineShape(rectBounds);

    //record its ID so that we can later refer to it
    const U16 circleID = circle->ID();
    ...
    //so that you can later refer to it.  The AddFillStyle function of fillStyle 
    //array returns the position so record that in a field called fillID
    const U32 blueFillID = circle->AddSolidFillStyle(new FColor(blue));
    ...
    circleShapeRecords[0] = new FShapeRecChange(false, true, true, false, true, 1703, 1702, 0, blueFillID, blackLineStyleID, nullptr, nullptr);
    ...
    auto *placeCircle = new FCTPlaceObject2(false, false, true, false, 3, circleID, nullptr, nullptr, 0, nullptr, 0);
    ...
    //construct the FDTDefineShape which will be the rectangle image
    auto *rectangle2 = new FDTDefineShape(rectBounds);

    //record its ID so that we can later refer to it
    const U16 rectangle2ID = rectangle2->ID();

    //construct a red FColor
    const FColor green = FColor(0, 0xff, 0);
    ...
    //so that you can later refer to it.  The AddFillStyle function of fillStyle 
    //array returns the position so record that in a field called fillID
    const U32 greenfillID = rectangle2->AddSolidFillStyle(new FColor(green));
    ...
    rectangle2ShapeRecords[0] = new FShapeRecChange(false, true, true, false, true, 7000, 3000, 0, greenfillID, blackLineStyleID, nullptr, nullptr);
    //Create straight edge object (just a stuct of info), store it in EdgeRecord
    ...
    //create a place object tag which puts the rectangle on the display list
    auto *placeRectangle2 = new FCTPlaceObject2(false, false, true, false, 4, rectangle2ID, nullptr, nullptr, 0, nullptr, 0);
