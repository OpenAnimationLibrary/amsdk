...
CTextRotator::CTextRotator(HDC hSourceDC, const LPCTSTR lpszText, const UINT uiHorzAlignment, const UINT uiVertAlignment) {
    m_hSourceDC = hSourceDC;
    m_hRotatedMemDC = nullptr;
...
void CTextRotator::AngleBlt(HDC hSourceDC, HDC hDestDC, const RECT rectSource, const UINT uiAngle) {
    const int iSourceWidth = rectSource.right - rectSource.left;
    const int iSourceHeight = rectSource.bottom - rectSource.top;

    if (uiAngle == 0) // Don't need to rotate text
    {
        BitBlt(hDestDC, 0, 0, iSourceWidth, iSourceHeight, hSourceDC, 0, 0, SRCCOPY);
    ...
    if (CNewLabel::IsWindowsNT2000()) // Windows NT/2000
    {
        ...
        PlgBlt(hDestDC, (const POINT *)&stPoints, hSourceDC, 0, 0, iSourceWidth, iSourceHeight, nullptr, 0, 0);
        return;
    }
    ...
    switch (uiAngle) {
        case 90:
            for (I = 0; I < iSourceWidth; I++)
                for (J = 0; J < iSourceHeight; J++) {
                    SetPixel(hDestDC, iSourceHeight - J - 1, I, GetPixel(hSourceDC, I, J));
                }
            break;
        ...
        case 180:
            for (I = 0; I < iSourceWidth; I++)
                for (J = 0; J < iSourceHeight; J++) {
                    SetPixel(hDestDC, I, J, GetPixel(hSourceDC, iSourceWidth - I - 1, iSourceHeight - J - 1));
                }
            break;
        ...
        case 270:
            for (I = 0; I < iSourceWidth; I++)
                for (J = 0; J < iSourceHeight; J++) {
                    SetPixel(hDestDC, J, I, GetPixel(hSourceDC, iSourceWidth - I - 1, J));
                }
            break;
        ...
        default:
            for (I = 0; I < iSourceWidth; I++)
                for (J = 0; J < iSourceHeight; J++) {
                    SetPixel(hDestDC, I, J, GetPixel(hSourceDC, I, J));
                }
            break;
    } // switch(uiAngle)
}
...
CSize CTextRotator::GetLengthlyTextSize(HDC hDC, CStringArray *parrayText) {
    ...
    for (int I = 0; I < parrayText->GetSize(); I++) {
        const CString &str = parrayText->GetAt(I);
        SIZE stSize;

        if (!GetTextExtentPoint(hDC, str, str.GetLength(), &stSize))
            continue;
...
BOOL CTextRotator::BitBltText(const int X, const int Y) const {
    if (!m_hRotatedMemDC)
        return FALSE;

    return BitBlt(m_hSourceDC, X, Y, m_rectRotated.Width(), m_rectRotated.Height(), m_hRotatedMemDC, 0, 0, SRCCOPY);
}

// --------------------------------------------------------------------------

BOOL CTextRotator::BitBltText(const CRect rectSource) {
    if (!m_hRotatedMemDC)
        return FALSE;

    const CRect rectRotatedSave = m_rectRotated;

    // Change rotated rectangle size to fit in source rectangle
    if (rectSource.Width() < m_rectRotated.Width())
        m_rectRotated.right = rectSource.Width();

    if (rectSource.Height() < m_rectRotated.Height())
        m_rectRotated.bottom = rectSource.Height();
    ...
    const BOOL bRet = BitBltText(X, Y); // Draw Text
...
void CTextRotator::GetRotatedDC(CDC *pDC) const {
    if (pDC)
...
void CTextRotator::GetRotatedRect(RECT *pstRect) const {
    const RECT rect = m_rectRotated;
...
BOOL CTextRotator::RotateText(const UINT uiAngle) {
    ...
    if (!pszText)
        return FALSE;
    strcpy_s(pszText,m_strText.GetLength(), m_strText);

    const char *psz = strtok(pszText, "\n");
    ...
    const HBRUSH hBrush = CreateSolidBrush(m_clrBackground);
...
void CNewLabel::DrawAlignedText(CDC *pDC, const LPCSTR lpszText, RECT stRect, UINT uiAlignment) const {
    UINT uiFormat = 0;
    ...
    const BOOL bSingleLine = !IsMultiLineString(lpszText);
    const BOOL bIsVTop = (m_uiAlignment & VERT_TOP) == VERT_TOP;
    const BOOL bIsVCenter = (m_uiAlignment & VERT_CENTER) == VERT_CENTER;
    const BOOL bIsVBottom = (m_uiAlignment & VERT_BOTTOM) == VERT_BOTTOM;
    ...
    if (bSingleLine || (!bSingleLine && bIsVTop)) {
        ...
        if (bIsVBottom) {
            const CSize cSize = pDC->GetTextExtent(lpszText);
...
int CNewLabel::GetWindowText(const LPTSTR lpszStringBuf, const int nMaxCount) {
    m_bBlockMessage = TRUE;

    const int iRet = CStatic::GetWindowText(lpszStringBuf, nMaxCount);
...
BOOL CNewLabel::IsMultiLineString(const LPCSTR lpszText) {
...
CString CNewLabel::MakeVerticalText(const LPCSTR lpszText) const {
    CString strDest;

    const CString strSource = lpszText ? CString(lpszText) : m_strText;
...
BOOL CNewLabel::ReconstructFont() {
    ...
    const BOOL bRet = m_cFont.CreateFontIndirect(&m_stLF);
...
void CNewLabel::SetWindowText(const LPCTSTR lpszString) {
    m_bBlockMessage = TRUE;
...
void CNewLabel::TrackMouse(const BOOL bHover, const BOOL bLeave) const {
...
CNewLabel &CNewLabel::SetBkColor(const COLORREF clr) {
...
CNewLabel &CNewLabel::SetHighLightColor(const COLORREF clr) {
    m_clrHightLight = clr;
...
CNewLabel &CNewLabel::SetTextBackColor(const COLORREF clr) {
...
CNewLabel &CNewLabel::SetTextColor(const COLORREF clr) {
...
CNewLabel &CNewLabel::SetFontBold(const BOOL bBold) {
...
CNewLabel &CNewLabel::SetFontItalic(const BOOL bItalic) {
    m_stLF.lfItalic = (UCHAR)bItalic;
    ReconstructFont();
    return *this;
}
...
CNewLabel &CNewLabel::SetFontName(const LPCTSTR szFaceName) {
    strcpy_s(m_stLF.lfFaceName, 32, szFaceName);
    ReconstructFont();
    return *this;
}
...
CNewLabel &CNewLabel::SetFontSize(const BOOL iSize) {
...
CNewLabel &CNewLabel::SetFontUnderline(const BOOL bUnderline) {
    m_stLF.lfUnderline = (UCHAR)bUnderline;
    ReconstructFont();
    return *this;
}
...
CNewLabel &CNewLabel::FlashBackground(const BOOL bActivate, const UINT uiTime) {
    if (m_bBackFlash)
        KillTimer(2);

    if (bActivate)
        SetTimer(2, uiTime, nullptr);
    else
...
CNewLabel &CNewLabel::FlashText(const BOOL bActivate, const UINT uiTime) {
    if (m_bTextFlash) {
...
void CNewLabel::ForceNoOwnerDraw(const BOOL bForce) {
...
BOOL CNewLabel::GetBorder() const {
    return (GetStyle() & WS_BORDER) == WS_BORDER ? TRUE : FALSE;
}

// --------------------------------------------------------------------------

UINT CNewLabel::GetHorzAlignment() const {
    if ((m_uiAlignment & HORZ_LEFT) == HORZ_LEFT)
        return HORZ_LEFT;
...
UINT CNewLabel::GetHorzAlignmentIndex() const {
    if ((m_uiAlignment & HORZ_LEFT) == HORZ_LEFT)
        return 0;
...
BOOL CNewLabel::GetSunken() const {
    return (GetExStyle() & WS_EX_STATICEDGE) == WS_EX_STATICEDGE ? TRUE : FALSE;
}

// --------------------------------------------------------------------------

CString CNewLabel::GetText() const {
    return m_strText;
}

// --------------------------------------------------------------------------

UINT CNewLabel::GetVertAlignment() const {
...
UINT CNewLabel::GetVertAlignmentIndex() const {
    if ((m_uiAlignment & VERT_TOP) == VERT_TOP)
        return 0;
...
CNewLabel &CNewLabel::SetAlignment(const UINT uiAlign) {
    DWORD dwRemove = 0;
    DWORD dwHorz = 0;
    DWORD dwVert = 0;
...
CNewLabel &CNewLabel::SetAngle(const UINT uiAngle) {
    m_uiAngle = uiAngle;
    RedrawWindow();
    return *this;
}
...
CNewLabel &CNewLabel::SetBackImage(const UINT nID) {
    if (m_hBackImage)
        DeleteObject(m_hBackImage);
...
CNewLabel &CNewLabel::SetBorder(const BOOL bSet) {
...
CNewLabel &CNewLabel::SetCursor(const HCURSOR hCursor) {
    m_hCursor = hCursor;
    return *this;
}
...
CNewLabel &CNewLabel::SetCursor(const UINT uiCursorID) {
...
CNewLabel &CNewLabel::SetDisabled(const BOOL bSet) {
...
CNewLabel &CNewLabel::SetLink(const BOOL bSet) {
    m_bLink = bSet;
    return *this;
}
...
CNewLabel &CNewLabel::SetLinkCursor(const HCURSOR hCursor) {
    m_hCursor = hCursor;
    return *this;
}
...
CNewLabel &CNewLabel::SetLinkCursor(const UINT uiCursorID) {
    m_hCursor = LoadCursor(AfxGetInstanceHandle(), MAKEINTRESOURCE(uiCursorID));
    return *this;
}
...
CNewLabel &CNewLabel::SetLowerCase(const BOOL bSet) {
    m_bLowerCase = bSet;
    m_bUpperCase = FALSE;
    RedrawWindow();
    return *this;
...
CNewLabel &CNewLabel::SetText(const LPCTSTR szText) {
    m_strText = szText;
...
CNewLabel &CNewLabel::SetSunken(const BOOL bSet) {
...
CNewLabel &CNewLabel::SetToolTipPosition(const int iPosition) {
...
CNewLabel &CNewLabel::SetToolTipText(const LPCTSTR lpszText) {
...
CNewLabel &CNewLabel::SetUpperCase(const BOOL bSet) {
    m_bLowerCase = FALSE;
    m_bUpperCase = bSet;
    RedrawWindow();
    return *this;
...
CNewLabel &CNewLabel::SetVerticalText(const BOOL bSet) {
...
CNewLabel &CNewLabel::ShowToolTipOnlyOnMouseClick(const BOOL bUse) {
...
CNewLabel &CNewLabel::StretchBitmap(const BOOL bStretch) {
...
CNewLabel &CNewLabel::UseBevelLine(const BOOL bUse) {
...
CNewLabel &CNewLabel::UseHighLighting(const BOOL bUse) {
...
CNewLabel &CNewLabel::UseHighLightingOnlyWithToolTip(const BOOL bUse) {
...
CNewLabel &CNewLabel::UseLeftMouseButtonForToolTip(const BOOL bUse) {
...
CNewLabel &CNewLabel::UseToolTip(const BOOL bUse) {
    m_bUseToolTip = bUse;
...
HBRUSH CNewLabel::CtlColor(CDC *pDC, const UINT nCtlColor) {
...
LRESULT CNewLabel::WindowProc(const UINT message, const WPARAM wParam, const LPARAM lParam) {
    if (message == WM_MOUSEHOVER) {
        if (m_bUseHighLighting) {
            m_clrSaveTextColor = m_clrTextColor;

            const CString str = m_cControlToolTip.GetText();

            if (!m_bUseHighLightingOnlyWithToolTip || (m_bUseHighLightingOnlyWithToolTip && m_bUseToolTip && !str.IsEmpty())) {
...
void CNewLabel::OnEnable(const BOOL bEnable) {
    SetRedraw(FALSE);
...
void CNewLabel::OnLButtonDown(const UINT nFlags, const CPoint point) {
    if (m_bLink) {
        ShellExecute(nullptr, "open", m_strText, nullptr, nullptr, SW_SHOWNORMAL);
    }
...
void CNewLabel::OnMouseMove(const UINT nFlags, const CPoint point) {
    if (!m_bInside) {
        m_bInside = TRUE;
...
BOOL CNewLabel::OnSetCursor(CWnd *pWnd, const UINT nHitTest, const UINT message) {
    if (m_hCursor) {
        ::SetCursor(m_hCursor);
        return TRUE;
    }
...
void CNewLabel::OnTimer(const UINT_PTR nIDEvent) {
    switch (nIDEvent) {
        case 1: // TEXT FLASH
            if (!m_bTextFlash)
                break;
            ...
            SetWindowText(m_bFlashTextState ? (CString)"" : m_strText);
...
void CNewLabel::OnPaint() {
    ...
    // Set Back Mode
    const int iOldMode = dc.SetBkMode(TRANSPARENT);
    ...
    if (m_bBackFlash && m_bFlashBackState) {
        const COLORREF clr = m_bUseDefaultBackColor ? m_clrBackColor / 2 : GetSysColor(COLOR_3DFACE);
    ...
    if (m_hBackImage) {
        ...
        if (m_bStretchBitmap) {
            dc.StretchBlt(0, 0, rectClient.Width(), rectClient.Height(), &memDC, 0, 0, stBitmap.bmWidth, stBitmap.bmHeight, SRCCOPY);
        } else {
    ...
    // Rotate Text
    if (m_hBackImage && m_uiAngle == 0 && (!IsMultiLineString(strText) || (IsMultiLineString(strText) && GetVertAlignment() == VERT_TOP))) {
        ...
        const BOOL bRet = cTextRotator.RotateText(m_uiAngle);

        if (bRet) {
            [[maybe_unused]] CRect rectRotated = cTextRotator.GetRotatedRect();
