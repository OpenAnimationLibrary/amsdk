...
CTextureMapDialog::CTextureMapDialog(CHxtApp *app, IEPolyModel *model) : CPluginDialog(IDD, nullptr) {
    ...
    //}}AFX_DATA_INIT
    m_copymaps = GetRegistryValue("3DS Plugin", "Copy Maps", TRUE);
    m_app = app;
    m_model = model;
}
...
void CTextureMapDialog::MakeUnique(String &filename, const int count/*=1*/) const {
    const int digits = (int)hash_math::log10((double)count) + 1;

    filename = filename.Left(7 - digits) + "_" + count;
...
BOOL CTextureMapDialog::OnInitDialog() {
    ...
    for (int i = 0; i < m_model->m_maplist.GetSize(); ++i) {
        ...
        m_app->m_maplist[i] = filename.Get();
    }
...
void CTextureMapDialog::OnOK() {
    CPluginDialog::OnOK();
    SetRegistryValue("3DS Plugin", "Copy Maps", m_copymaps);
}
