// RC  10/1/2001  \Randy90\Plugin\Textures\InkStroke\Evaluate.cpp
...
//    Vector2 re(m_tx->GetValue(), m_ty->GetValue());
    ...
    float x = repeat(evalp.x, m_xs->GetValue()/100.F);
    float y = repeat(evalp.y, m_ys->GetValue()/100.F);
    if (m_bitmap->GetClip()) {
    ...
    diffuse.m_green = diffuse.m_green < 0.F ? 0.F : diffuse.m_green;
    diffuse.m_green = diffuse.m_green > 1.F ? 1.F : diffuse.m_green;

    const float cval = diffuse.m_green; //use green.
    alpha += 1.F - cval * rough;
    alpha = alpha > 1.F ? 1.F : alpha;
    color = alpha * color + (1.F - alpha) * color2;
    color = mix * color + (1.F - mix) * origcolor;
